...
void RequestListDialog::slotRemoveRequestResult(int result) {
    if (result == REMOVE_REQUEST_CHANNEL_SUCCESS_ANSWER && is_channel_) {
        removeRequest(removed_request_username_);
    } else if (result == REMOVE_REQUEST_CHANNEL_FAILED_ANSWER && is_channel_) {
        std::unique_ptr<InfoDialog> dialog_ptr = std::make_unique<InfoDialog>(nullptr, "Что-то пошло не так при попытке удалить запрос.");
        dialog_ptr->exec();
    }

    if (result == REMOVE_REQUEST_CHAT_SUCCESS_ANSWER && !is_channel_) {
        removeRequest(removed_request_username_);
    } else if (result == REMOVE_REQUEST_CHAT_FAILED_ANSWER && !is_channel_) {
        std::unique_ptr<InfoDialog> dialog_ptr = std::make_unique<InfoDialog>(nullptr, "Что-то пошло не так при попытке удалить запрос.");
        dialog_ptr->exec();
    }
}

void RequestListDialog::slotChannelAddParticipantResult(int result) {
    if (result == ADD_PARTICIPANT_TO_PRIVATE_CHANNEL_SUCCESS_ANSWER) {
        removeRequest(removed_request_username_);
    }
}

void RequestListDialog::slotChatAddParticipantResult(int result) {
    if (result == ADD_PARTICIPANT_TO_PRIVATE_CHAT_SUCCESS_ANSWER && !is_channel_) {
        removeRequest(removed_request_username_);
    }
}
...
void RequestListDialog::setupConnections() {
    if (is_channel_) {
        connect(ClientSingleton::get_client(), &Client::add_private_channel_participant_result,       this, &RequestListDialog::slotChannelAddParticipantResult);
        connect(ClientSingleton::get_client(), &Client::remove_request_channel_result,                this, &RequestListDialog::slotRemoveRequestResult);
        disconnect(ClientSingleton::get_client(), &Client::add_private_chat_participant_result,       this, &RequestListDialog::slotChatAddParticipantResult);
        disconnect(ClientSingleton::get_client(), &Client::remove_request_chat_result,                this, &RequestListDialog::slotRemoveRequestResult);
    } else {
        connect(ClientSingleton::get_client(), &Client::add_private_chat_participant_result,          this, &RequestListDialog::slotChatAddParticipantResult);
        connect(ClientSingleton::get_client(), &Client::remove_request_chat_result,                   this, &RequestListDialog::slotRemoveRequestResult);
        disconnect(ClientSingleton::get_client(), &Client::add_private_channel_participant_result,    this, &RequestListDialog::slotChannelAddParticipantResult);
        disconnect(ClientSingleton::get_client(), &Client::remove_request_channel_result,             this, &RequestListDialog::slotRemoveRequestResult);
    }
}
